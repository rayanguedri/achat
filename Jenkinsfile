pipeline {
    ...
    stages {
        stage('Checkout GIT') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: 'ghassen-souissi']], userRemoteConfigs: [[url: 'https://github.com/rayanguedri/achat.git']]])
            }
        }
  ...
  stage('Curl Grafana Dashboard') {
    steps {
        ...
        stage('Start MySQL Container') {
            steps {
                script {
                    def gitRepo = checkout([$class: 'GitSCM', branches: [[name: 'ghassen-souissi']], userRemoteConfigs: [[url: 'https://github.com/rayanguedri/achat/tree/ghassen-souissi']]])
